...
export default function ContentDownload(props: ContentDownloadProps) {
  const dict = useDownloadDictionaries().dict;
  const driverColumns = React.useMemo<
    ColumnDef<{ [key: string]: string }>[]
  >(() => {
    return dict.download.drivers.columns.map((item, index) => {
      return {
        accessorKey: item.id,
        header: (headerProps) => {
          if (item.id === "link") {
  ...
  >(() => {
    return dict.download.documents.columns.map((item, index) => {
      return {
        accessorKey: item.id,
        header: (headerProps) => {
          if (item.id === "link") {
